...
export async function fetchComments(page: Page): Promise<CommentData | null> {
  ...
  const contentDiv = $('#comment-list-app > div.tw-comment-list-view.tw-player-page__comment__list > div.tw-comment-list-view__scroller');
  ...
  const name: string[] = contentDiv.find('.tw-comment-item-name').map((i, el) => $(el).text()).get();
  const id: string[] = contentDiv.find('.tw-comment-item-screen-id').map((i, el) => $(el).text()).get();
  const comment: string[] = contentDiv.find('.tw-comment-item-comment').map((i, el) => $(el).text()).get();
  const date: string[] = contentDiv.find('.tw-comment-item-date').map((i, el) => $(el).text()).get();
...
export async function collectComments(page: Page, userId: string, flag: boolean = true): Promise<void> {
  await page.waitForSelector(
    "#comment-list-app > div.tw-comment-list-view.tw-player-page__comment__list > div.tw-comment-list-view__scroller"
  );

  let oldId = "", oldComment = "";
  while (flag) {
